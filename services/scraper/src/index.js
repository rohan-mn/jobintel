...
async function fetchRemoteOkJobs() {
    ...
    const res = await axios_1.default.get("https://remoteok.com/api", {
        headers: {
    ...
    return data.filter((x) => x && typeof x === "object" && ("position" in x || "company" in x || "url" in x));
}
async function main() {
    ...
    for (let i = 0; i < mapped.length; i += chunkSize) {
        ...
        const res = await axios_1.default.post(url, { jobs: chunk }, {
